...
void lcd_init(void)
{
    ...
    GPIO_Init(LCD_CTRL_GPIOx, GPIO_MODE_OUTPUT, LCD_EN_PIN);
    GPIO_Init(LCD_CTRL_GPIOx, GPIO_MODE_OUTPUT, LCD_RW_PIN);
    GPIO_Init(LCD_CTRL_GPIOx, GPIO_MODE_OUTPUT, LCD_RS_PIN);

    GPIO_Write_Pin(LCD_CTRL_GPIOx, LCD_EN_PIN, GPIO_PIN_LOW);
    GPIO_Write_Pin(LCD_CTRL_GPIOx, LCD_RW_PIN, GPIO_PIN_LOW);
    GPIO_Write_Pin(LCD_CTRL_GPIOx, LCD_RS_PIN, GPIO_PIN_LOW);
...
#ifdef LCD_EIGHT_BIT_MODE
    LCD_DATA_PORT = data;
    GPIO_Write_Pin(LCD_CTRL_GPIOx, LCD_RS_PIN, GPIO_PIN_HIGH);
    GPIO_Write_Pin(LCD_CTRL_GPIOx, LCD_RW_PIN, GPIO_PIN_LOW);
    lcd_Enable_Signal();
#endif
...
{
    GPIO_Write_Pin(LCD_CTRL_GPIOx, LCD_EN_PIN, GPIO_PIN_HIGH);
    wait_ms(5);
    GPIO_Write_Pin(LCD_CTRL_GPIOx, LCD_EN_PIN, GPIO_PIN_LOW);
}
...
uint8_t lcd_Is_Busy(void)
{
    ...
    wait_ms(1);
    GPIO_Write_Pin(LCD_CTRL_GPIOx, LCD_RW_PIN, GPIO_PIN_HIGH);
    GPIO_Write_Pin(LCD_CTRL_GPIOx, LCD_RS_PIN, GPIO_PIN_LOW);
    lcd_Enable_Signal();
